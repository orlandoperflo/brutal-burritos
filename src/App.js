...
import { getFirestore, doc, setDoc, getDoc, collection, addDoc, onSnapshot, query, where, deleteDoc, updateDoc, serverTimestamp, writeBatch, getDocs } from 'firebase/firestore';
...
if (firebaseConfig && firebaseConfig.apiKey) { try { app = initializeApp(firebaseConfig); auth = getAuth(app); db = getFirestore(app); } catch (e) { console.error("Error initializing Firebase:", e); auth = { currentUser: null, onAuthStateChanged: () => () => {}, signInAnonymously: async () => ({ user: { uid: 'anon-user-' + crypto.randomUUID() } }) }; db = { collection: () => ({ onSnapshot: () => () => {}, doc: () => ({ get: async () => ({ exists: () => false, data: () => null }) }) }) }; }
} else { console.warn("Firebase config not found. App will run with limited functionality."); auth = { currentUser: null, onAuthStateChanged: () => () => {}, signInAnonymously: async () => ({ user: { uid: 'anon-user-' + crypto.randomUUID() } }) }; db = { collection: () => ({ onSnapshot: () => () => {}, doc: () => ({ get: async () => ({ exists: () => false, data: () => null }) }) }) }; }
const appId = typeof __app_id !== 'undefined' ? __app_id : 'brutal-burritos-app';
...
function App() {
  ...
  useEffect(() => { 
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => { if (user) { setUserId(user.uid); } else { try { if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) { await signInWithCustomToken(auth, __initial_auth_token); } else { await signInAnonymously(auth); } } catch (error) { console.error("Error signing in: ", error); setUserId('anonymous-' + crypto.randomUUID()); } } setIsAuthReady(true); });
    return () => unsubscribeAuth();
  }, []); 
  useEffect(() => { if (isAuthReady && firebaseConfig && firebaseConfig.apiKey && db) { setupProductsInFirestore(); } }, [isAuthReady]);
  useEffect(() => { 
    if (!(firebaseConfig && firebaseConfig.apiKey && db)) { return; } 
    if (!isAuthReady) return; 
    const productsRef = collection(db, `/artifacts/${appId}/public/data/products`);
    const unsubscribeProducts = onSnapshot(productsRef, (snapshot) => { const fetchedProducts = snapshot.docs.map(docSnap => ({ id: docSnap.id, ...docSnap.data() })); if (fetchedProducts.length > 0) { setProducts(fetchedProducts); } else { console.log("No products in Firestore, using initial mock. Consider running setup."); } }, (error) => { console.error("Error fetching products: ", error); });
    return () => unsubscribeProducts();
  }, [isAuthReady, appId]);
  useEffect(() => { 
    if (!userId || !isAuthReady || !(firebaseConfig && firebaseConfig.apiKey && db)) { return; }
  ...
  const handleAddToCart = async (productData, quantity, customizations = {}, isComplementary = false, editingCartItemId = null) => { 
    if (!isAuthReady) { console.warn("Auth not ready."); return; } 
    ...
    if (!(firebaseConfig && firebaseConfig.apiKey && userId && db)) { 
        console.log("Local cart operation."); 
        setCartItems(prevItems => {
            if (editingCartItemId) { 
                return prevItems.map(item => item.id === editingCartItemId ? { ...item, ...cartItemPayload, addedAt: item.addedAt } : item);
            } else { 
                const localId = crypto.randomUUID(); 
                const existingIdx = prevItems.findIndex(i => i.productId === productData.id && JSON.stringify(i.customizations || {}) === JSON.stringify(customizations || {})); 
                if (existingIdx > -1) { const upd = [...prevItems]; upd[existingIdx].quantity += quantity; return upd; } 
                else { return [...prevItems, { ...cartItemPayload, id: localId, addedAt: new Date() }]; }
            }
        });
    ...
    try { 
        const cartRef = collection(db, `/artifacts/${appId}/users/${userId}/cartItems`); 
        if (editingCartItemId) {
            const itemDocRef = doc(db, `/artifacts/${appId}/users/${userId}/cartItems`, editingCartItemId);
            await updateDoc(itemDocRef, { ...cartItemPayload, modifiedAt: serverTimestamp() }); 
            console.log(`${productData.name} updated in Firestore cart.`);
        } else {
            const existingItem = cartItems.find(i => i.productId === productData.id && JSON.stringify(i.customizations || {}) === JSON.stringify(customizations || {})); 
            if (existingItem) { 
                const itemDocRef = doc(db, `/artifacts/${appId}/users/${userId}/cartItems`, existingItem.id); 
                await updateDoc(itemDocRef, { quantity: existingItem.quantity + quantity, price: productData.price, modifiedAt: serverTimestamp() }); 
                console.log(`${productData.name} quantity updated.`); 
            } else { 
                await addDoc(cartRef, {...cartItemPayload, addedAt: serverTimestamp()}); 
                console.log(`${productData.name} added to Firestore cart.`); 
            } 
        }
        if(isComplementary && currentPage === 'cart') { console.log("Complementary item added, staying on cart page.");} 
    } 
    catch (error) { console.error("Error adding/updating cart: ", error); console.log("Error adding product to cart."); }
  };
  ...
  const handleUpdateQuantity = async (cartItemId, newQuantity) => { if (newQuantity < 1) { handleRemoveItem(cartItemId); return; } if (!(firebaseConfig && firebaseConfig.apiKey && userId && db)) { setCartItems(prev => prev.map(i => i.id === cartItemId ? {...i, quantity: newQuantity} : i)); return; } try { await updateDoc(doc(db, `/artifacts/${appId}/users/${userId}/cartItems`, cartItemId), { quantity: newQuantity }); } catch (error) { console.error("Error updating quantity: ", error); } };
  const handleRemoveItem = async (cartItemId) => { if (!(firebaseConfig && firebaseConfig.apiKey && userId && db)) { setCartItems(prev => prev.filter(i => i.id !== cartItemId)); return; } try { await deleteDoc(doc(db, `/artifacts/${appId}/users/${userId}/cartItems`, cartItemId)); } catch (error) { console.error("Error removing item: ", error); } };
  const handleClearCart = async () => { if (!(firebaseConfig && firebaseConfig.apiKey && userId && db)) { setCartItems([]); return; } try { const batch = writeBatch(db); cartItems.forEach(item => batch.delete(doc(collection(db, `/artifacts/${appId}/users/${userId}/cartItems`), item.id))); await batch.commit(); } catch (error) { console.error("Error clearing cart: ", error); } };
  
  const renderPage = () => {
    if (appLoading && !(firebaseConfig && firebaseConfig.apiKey)) { return <div className="flex justify-center items-center h-screen"><p className="text-xl animate-pulse">Cargando Brutal Burritos...</p></div>; }
    switch (currentPage) {
